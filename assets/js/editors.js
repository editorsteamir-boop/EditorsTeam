(() => {
  ...
  async function loadEditors() {
    try {
      const response = await fetch("./data/editors.json?v=15.0.0", { cache: "no-store" });
  ...
  function initializeEditors() {
    ...
    const embedded=Array.isArray(window.__ET_EDITORS__)?window.__ET_EDITORS__.map(normalizeEditor):[];
    editorsData=cached.length?cached:embedded;
    renderEditors();
    // Network refresh is intentionally detached from initial rendering.
    loadEditors();
  }
