...
(() => {
  ...
  async function loadFont(item, card){
    ...
    if(loaded.has(item.id)){
      await loaded.get(item.id);
      applyPreview(card,family);
      return family;
    }
    const task=(async()=>{
      const face=new FontFace(family,`url("${fontUrl(item.file_name)}")`,{display:'swap'});
  ...
  function applyPreview(card,family){
    if(!card)return;
    const text=card.querySelector('.fonto-font-preview-text');
    if(!text)return;
    text.style.fontFamily=`"${family}"`;
    card.classList.add('fonto-font-card-ready');
  ...
  function fitText(el){
    ...
    while(size>15 && (el.scrollWidth>maxW || el.scrollHeight>maxH)){
  ...
  function makeCard(item){
    ...
    card.type='button';card.className='fonto-font-card';card.dataset.fontId=item.id;
    ...
    card.addEventListener('click',async()=>{
      try{
        const family=await loadFont(item,card);
        document.querySelectorAll('.fonto-font-card.selected').forEach(x=>x.classList.remove('selected'));
        card.classList.add('selected');
        window.dispatchEvent(new CustomEvent('fonto:font-selected',{detail:{family,name:item.name,fileName:item.file_name}}));
      }catch(_){/* keep current editor font on failed load */}
    });
  ...
  async function init(){
    const host=document.getElementById('fontoFontPreviewList');
    ...
    observer=new IntersectionObserver(entries=>{
      entries.forEach(entry=>{
        ...
        const card=entry.target,item=card._fontItem;
        observer.unobserve(card);
        if(item)loadFont(item,card).catch(()=>{});
      });
    },{root:host,rootMargin:'180px 240px',threshold:.01});
    try{
      const r=await fetch(`${SUPABASE_URL}/rest/v1/fonto_fonts?select=id,name,file_name,category&order=category.asc,name.asc`,{headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${SUPABASE_KEY}`}});
      if(!r.ok)throw new Error(`fonts ${r.status}`);
      const items=await r.json();host.innerHTML='';
      items.forEach(item=>{const card=makeCard(item);card._fontItem=item;host.appendChild(card);});
  ...
  window.initFontoFontPreviews=init;
})();