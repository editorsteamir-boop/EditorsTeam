(() => {
  ...
  async function imageToWebP(file, quality=0.86){
    ...
    bitmap.close?.();
    const blob=await new Promise((resolve,reject)=>canvas.toBlob(b=>b?resolve(b):reject(new Error("تبدیل تصویر به WebP انجام نشد.")),"image/webp",quality));
    const base=(file.name||"image").replace(/\.[^.]+$/,'');
    return new File([blob],`${base}.webp`,{type:"image/webp",lastModified:Date.now()});
  }
  async function ensureSquareImage(file){
    if(!file || !String(file.type||"").startsWith("image/")) throw new Error("فایل انتخاب‌شده باید تصویر باشد.");
    const bitmap=await createImageBitmap(file);
    const ok=bitmap.width===bitmap.height; bitmap.close?.();
    if(!ok) throw new Error("ابعاد تصویر ادیتور باید دقیقاً نسبت ۱ به ۱ باشد.");
  }
  ...
  $("editorImage").addEventListener("change",async e=>{const f=e.target.files[0];if(!f)return;try{await ensureSquareImage(f);state.editorFile=f;$("avatarPreview").src=URL.createObjectURL(f);toast("تصویر معتبر است؛ هنگام ذخیره به WebP تبدیل می‌شود");}catch(err){e.target.value="";alert(err.message);}});
