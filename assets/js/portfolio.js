(() => {
  ...
  function localEditorData(id){
    const embedded=Array.isArray(window.__ET_EDITORS__)?window.__ET_EDITORS__:[];
    return [...readStoredEditors(),...embedded].find(x=>String(x?.id)===String(id))||null;
  }
  function refreshEditorData(id){
    fetch("./data/editors.json?v=15.0.0",{cache:"no-store"}).then(r=>r.ok?r.json():Promise.reject()).then(d=>{
      if(!Array.isArray(d))return;
      try{localStorage.setItem("editorsTeam.editors.v1",JSON.stringify(d));}catch{}
      const fresh=d.find(x=>String(x?.id)===String(id));
      if(fresh){currentEditor=fresh;currentMedia=normalizeMedia(fresh);renderMedia();}
    }).catch(()=>{});
  }
  async function init(){const modal=$("portfolioModal");if(modal){modal.hidden=true;modal.classList.remove("active");}const id=new URLSearchParams(location.search).get("id");try{currentEditor=localEditorData(id);if(!currentEditor)throw 0;try{sessionStorage.setItem("editorsTeam.selectedEditor",JSON.stringify(currentEditor));}catch{}$("portfolioName").textContent=currentEditor.fullName||"ادیتور";$("portfolioAvatar").src=currentEditor.image||DEFAULT_AVATAR;$("portfolioAvatar").onerror=function(){this.src=DEFAULT_AVATAR};document.title=`نمونه‌کارهای ${currentEditor.fullName||"ادیتور"}`;currentMedia=normalizeMedia(currentEditor);renderMedia();refreshEditorData(id);if(window.SalesStore)window.SalesStore.load().then(refreshSaleControls).catch(()=>{});}catch{$("portfolioName").textContent="ادیتور پیدا نشد";$("portfolioAvatar").src=DEFAULT_AVATAR;$("portfolioGallery").innerHTML='<div class="portfolio-empty">اطلاعات این ادیتور در دسترس نیست.</div>';}}
