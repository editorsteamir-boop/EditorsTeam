(() => {
  ...
  function loadEditorLocal(id){
    let selected=null;try{selected=JSON.parse(sessionStorage.getItem("editorsTeam.selectedEditor")||"null")}catch{}
    if(selected&&String(selected.id)===String(id))return selected;
    return readStoredEditors().find(x=>String(x?.id)===String(id))||null;
  }
  function refreshEditor(id){
    fetch("./data/editors.json?v=16.0.0",{cache:"no-cache"}).then(r=>r.ok?r.json():Promise.reject()).then(d=>{
      if(!Array.isArray(d))return;
      try{localStorage.setItem("editorsTeam.editors.v1",JSON.stringify(d));}catch{}
      const fresh=d.find(x=>String(x?.id)===String(id)); if(!fresh)return;
      currentEditor=fresh; currentMedia=normalizeMedia(fresh); renderMedia();
    }).catch(()=>{});
  }
  async function init(){const modal=$("portfolioModal");if(modal){modal.hidden=true;modal.classList.remove("active");}const id=new URLSearchParams(location.search).get("id");try{currentEditor=loadEditorLocal(id);if(!currentEditor)throw 0;try{sessionStorage.setItem("editorsTeam.selectedEditor",JSON.stringify(currentEditor));}catch{}$("portfolioName").textContent=currentEditor.fullName||"ادیتور";$("portfolioAvatar").src=currentEditor.image||DEFAULT_AVATAR;$("portfolioAvatar").onerror=function(){this.src=DEFAULT_AVATAR};document.title=`نمونه‌کارهای ${currentEditor.fullName||"ادیتور"}`;currentMedia=normalizeMedia(currentEditor);renderMedia();refreshEditor(id);if(window.SalesStore)window.SalesStore.load().then(refreshSaleControls).catch(()=>{});}catch{$("portfolioName").textContent="ادیتور پیدا نشد";$("portfolioAvatar").src=DEFAULT_AVATAR;$("portfolioGallery").innerHTML='<div class="portfolio-empty">اطلاعات این ادیتور در دسترس نیست.</div>';}}
