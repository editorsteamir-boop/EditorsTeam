(() => {
  ...
  function openProject(id){
    ...
    gallery.innerHTML=(p.images||[]).map((im,i)=>{const eager=i<2;return `<article class="sellable-card"><img class="gallery-img" src="${esc(im.src)}" alt="${esc(im.alt||`${p.title} ${i+1}`)}" data-gallery-image loading="${eager?'eager':'lazy'}" decoding="async" ${eager?'fetchpriority="high"':''}>${saleBox(p,im,i)}</article>`}).join("")||'<div class="home-note">هنوز تصویری برای این پروژه ثبت نشده است.</div>';
